...
bucle3: bge $t4, $t5, fin3
		move $t7, $t4
		add $t6, $t6, $t7
		addi $t4, $t4, 32
        b bucle3
   fin3:
   	li $v0 0 #Resultado = 0 si todo ha ido bien
        lw $t0, ($sp)
        lw $t1, 4($sp)
        lw $t2, 8($sp)
        lw $t3, 12($sp)
...
# M -> Número de filas.
# N -> Número de columnas.
# B -> dirección de inicio de una matriz de números enteros de dimensión MxN.
# i -> entero correspondiente a la fila del primer elemento de la matriz
# j -> entero correspondiente a la columna del primer elemento de la matriz
# k -> entero correspondiente a la fila del último elemento de la matriz 
# l ->  entero correspondiente a la columna del último elemento de la matriz
#Devuelve el número de elementos de ambas matrices que coinciden en la misma posición (c,d)
